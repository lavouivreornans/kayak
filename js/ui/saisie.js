...
import { CLASSES_RIVIERE } from "../classes-riviere.js";
...
// Heure actuelle arrondie au quart d'heure le plus proche (ex : 14:37 -> 14:30).
function nowQuartHeure() {
  const d = new Date();
  d.setSeconds(0, 0);
  d.setMinutes(Math.round(d.getMinutes() / 15) * 15); // 60 -> bascule proprement à l'heure suivante
  return d.toTimeString().slice(0, 5);
}

const MINUTES_QUART = ["00", "15", "30", "45"];

/** Sélecteurs heure/minute (par quart d'heure) pour un champ heureDebut/heureFin. */
function renderHeureSelect(name, valeur) {
  const [h, m] = (valeur || "").split(":");
  const minuteProche = m != null ? MINUTES_QUART.reduce((a, b) => (Math.abs(b - m) < Math.abs(a - m) ? b : a)) : "";
  const heures = Array.from({ length: 24 }, (_, i) => String(i).padStart(2, "0"));
  return `
    <div class="time-select">
      <select name="${name}H" aria-label="Heure">
        <option value="">—</option>
        ${heures.map((hh) => `<option value="${hh}" ${h === hh ? "selected" : ""}>${hh}</option>`).join("")}
      </select>
      <span>:</span>
      <select name="${name}M" aria-label="Minutes">
        <option value="">—</option>
        ${MINUTES_QUART.map((mm) => `<option value="${mm}" ${minuteProche === mm ? "selected" : ""}>${mm}</option>`).join("")}
      </select>
    </div>
  `;
}

function combinerHeure(h, m) {
  return h && m ? `${h}:${m}` : "";
}
  ...
  container.innerHTML = `
    <form id="form-seance" class="form-saisie" novalidate>
      <section class="card">
        ...
        <label>Classe de la rivière (difficulté)
          <select name="classeRiviere">
            <option value="">—</option>
            ${CLASSES_RIVIERE.map((c) => `<option value="${c}" ${seance?.classeRiviere === c ? "selected" : ""}>${c}</option>`).join("")}
          </select>
        </label>
        <div class="row">
      ...
      <section class="card">
        ...
        <div class="row">
          ...
          <label>Heure début
            ${renderHeureSelect("heureDebut", seance?.heureDebut || "")}
          </label>
          <label>Heure fin
            ${renderHeureSelect("heureFin", seance?.heureFin || nowQuartHeure())}
          </label>
        </div>
      </section>
...
function onSubmit(container) {
  return async (e) => {
    ...
    const seance = {
      ...
      date: fd.get("date"),
      heureDebut: combinerHeure(fd.get("heureDebutH"), fd.get("heureDebutM")),
      heureFin: combinerHeure(fd.get("heureFinH"), fd.get("heureFinM")),
      niveauEau: fd.get("niveauEau"),
      classeRiviere: fd.get("classeRiviere") || null,
      notes: fd.get("notes"),
      lieu: {
