...
async function renderCarte(container, seances) {
  ...
  if (!groupes.length) {
    el.innerHTML = `<p class="hint">Aucun spot géolocalisé pour la sélection actuelle (période/filtres) — renseignez au moins la rivière dans une séance pour la faire apparaître ici.</p>`;
    return;
  }
