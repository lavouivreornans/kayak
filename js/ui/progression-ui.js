...
/** Résumé de pratique : sorties, rivières, débit max, classe max + carte — filtré par période et, au clic, par mois. */
function renderResumePratique(el) {
  ...
  const seances = filtreMoisPersonne ? dansPeriode.filter((s) => moisOf(s) === filtreMoisPersonne) : dansPeriode;

  renderChartMoisPersonne(el, dansPeriode);
