...
// Période et filtre mois du résumé de pratique — partagés entre les
// personnes consultées, comme sur le tableau de bord club.
const periodePersonne = { preset: "tout", debut: null, fin: null };
let filtreMoisPersonne = null;
let filtreRivierePersonne = null;
...
function riviereOf(s) {
  return s.lieu?.riviere || s.lieu?.nom || null;
}
...
export async function render(container, { adherentId = null } = {}) {
  ...
  if (adherentId) {
    ...
    niveauOuvert = null;
    filtreMoisPersonne = null;
    filtreRivierePersonne = null;
  }
...
function renderContenu(container) {
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <div id="chips-personne" class="filter-chips"></div>
      <div class="stat-cards" id="stat-cards-personne"></div>
      <div class="charts-row">
        <div class="chart-card">
          <h3>Sorties par mois <span class="hint">(cliquer un mois)</span></h3>
          <div id="chart-mois-personne"></div>
        </div>
        <div class="chart-card">
...
/** Graphique "sorties par mois" de la personne, cliquable pour filtrer le reste du résumé sur un mois. */
function renderChartMoisPersonne(el, seances) {
  const parMois = {};
  seances.forEach((s) => {
    const m = moisOf(s);
  ...
  const chartEl = el.querySelector("#chart-mois-personne");
  chartEl.innerHTML = "";
  chartEl.appendChild(
    charts.barChart(data, {
      width: 300,
      height: 160,
      selectedKey: filtreMoisPersonne,
      onClick: (d) => {
        filtreMoisPersonne = filtreMoisPersonne === d.key ? null : d.key;
        renderResumePratique(el);
      },
      onReset: () => {
        filtreMoisPersonne = null;
...
/** Résumé de pratique : sorties, rivières, débit max, classe max + carte — filtré par période et, au clic, par mois/rivière. */
function renderResumePratique(el) {
  const toutes = sessionsApi.seancesParParticipant(adherentSelectionne);
  const dansPeriode = filtrerParPeriode(toutes, periodePersonne);
  const pourMois = dansPeriode.filter((s) => !filtreRivierePersonne || riviereOf(s) === filtreRivierePersonne);
  const pourRiviere = dansPeriode.filter((s) => !filtreMoisPersonne || moisOf(s) === filtreMoisPersonne);
  const seances = dansPeriode.filter(
    (s) =>
      (!filtreMoisPersonne || moisOf(s) === filtreMoisPersonne) &&
      (!filtreRivierePersonne || riviereOf(s) === filtreRivierePersonne)
  );

  renderChartMoisPersonne(el, pourMois);
  renderChipsPersonne(el);

  const rivieresUniques = [...new Set(seances.map(riviereOf).filter(Boolean))];
  ...
  const onClickRiviere = (d) => {
    filtreRivierePersonne = filtreRivierePersonne === d.label ? null : d.label;
    renderResumePratique(el);
  };
  ...
  if (data.length) {
    chartEl.appendChild(
      charts.donutChart(data, { size: 140, selectedKey: filtreRivierePersonne, onClick: onClickRiviere, onReset: onResetRiviere })
    );
    chartEl.appendChild(charts.legende(data, { selectedKey: filtreRivierePersonne, onClick: onClickRiviere }));
  } else {
    chartEl.innerHTML = `<p class="hint">Aucune séance sur cette période.</p>`;
  }

  renderCarteSpots(el.querySelector("#map-personne"), seances, {
    messageVide: "Aucun spot géolocalisé pour cette personne sur la sélection actuelle.",
  });
}

function renderChipsPersonne(el) {
  const zone = el.querySelector("#chips-personne");
  const chips = [];
  if (filtreMoisPersonne) {
    const [y, m] = filtreMoisPersonne.split("-").map(Number);
    const MOIS_LONGS = [
      "Janvier", "Février", "Mars", "Avril", "Mai", "Juin",
      "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre",
    ];
    chips.push({ texte: `📅 ${MOIS_LONGS[m - 1]} ${y}`, clear: () => (filtreMoisPersonne = null) });
  }
  if (filtreRivierePersonne) {
    chips.push({ texte: `🌊 ${filtreRivierePersonne}`, clear: () => (filtreRivierePersonne = null) });
  }
  if (!chips.length) {
    zone.innerHTML = "";
    return;
  }
  zone.innerHTML = chips.map((c, i) => `<button type="button" class="chip-filtre" data-i="${i}">${escHtml(c.texte)} ✕</button>`).join("");
  zone.querySelectorAll(".chip-filtre").forEach((btn) => {
    btn.addEventListener("click", () => {
