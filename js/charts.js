...
export function donutChart(data, { size = 160, thickness = 26, onClick = null, selectedKey = null, onReset = null } = {}) {
  ...
  data.forEach((d, i) => {
    ...
    // Nom directement dans la part du donut, courbé le long de l'anneau —
    // évite d'avoir à faire l'aller-retour avec la légende. Tronqué (ou
    // carrément masqué) si la part est trop fine pour l'accueillir.
    if (frac > 0.035) {
      const midAngle = (angleStart + angleEnd) / 2;
      // Sur la moitié basse du cercle, un texte suivant l'arc dans le sens
      // horaire se retrouverait la tête en bas : on parcourt l'arc à
      // l'envers (même tracé géométrique, sens inverse) pour qu'il reste lisible.
      const bas = Math.sin(midAngle) > 0;
      const idChemin = `donut-label-${Math.random().toString(36).slice(2, 9)}`;
      const chemin = svgEl("path", {
        id: idChemin,
        d: bas ? `M ${x2} ${y2} A ${r} ${r} 0 ${largeArc} 0 ${x1} ${y1}` : `M ${x1} ${y1} A ${r} ${r} 0 ${largeArc} 1 ${x2} ${y2}`,
        fill: "none",
      });
      svg.appendChild(chemin);

      const taillePolice = Math.max(5.5, Math.min(8, thickness * 0.34));
      const longueurArcPx = frac * 2 * Math.PI * r;
      const maxCars = Math.floor((longueurArcPx * 0.82) / (taillePolice * 0.58));
      if (maxCars >= 2) {
        const libelle = d.label.length > maxCars ? d.label.slice(0, maxCars - 1) + "…" : d.label;
        const texte = svgEl("text", { class: "chart-donut-label", "text-anchor": "middle" });
        texte.style.fontSize = `${taillePolice}px`;
        texte.style.opacity = aSelection && !estSelectionne ? "0.35" : "1";
        const textPath = document.createElementNS("http://www.w3.org/2000/svg", "textPath");
        textPath.setAttribute("href", `#${idChemin}`);
        textPath.setAttributeNS("http://www.w3.org/1999/xlink", "href", `#${idChemin}`);
        textPath.setAttribute("startOffset", "50%");
        textPath.setAttribute("text-anchor", "middle");
        textPath.textContent = libelle;
        texte.appendChild(textPath);
